...
describe('Clients API', () => {
  it('POST /api/store should create new user with 201 status', async () => {
  ...
  it('POST /api/store should return error descriptions with 422 status on validation error', async () => {
  ...
  it('GET /api/store should return a list of store with 200 status', async () => {
    const requests = [
      axios.post('', client),
      axios.post('', client),
      axios.post('', client),
  ...
  it('GET /api/store/{id} should return a client object with 200 status', async () => {
  ...
  it('GET /api/store/{id} should fail with 404 status for inexistent client ID', async () => {
    const res = await axios.get('i don\'t know this guy');
    expect(res.status).toBe(404);
  });

  it('PATCH /api/store/{id} should partially update a client with 200 status', async () => {
  ...
  it('PATCH /api/store/{id} should return error descriptions with 422 status on validation error', async () => {
    const {data: {id}} = await axios.post('', client);
  ...
  it('PATCH /api/store/{id} should fail with 404 status for inexistent client ID', async () => {
    const res = await axios.patch('i don\'t know this guy', {name: 'John'});
    expect(res.status).toBe(404);
  });

  it('DELETE /api/store/{id} should delete a client with 200 status', async () => {
    const {data: {id}} = await axios.post('', client);
